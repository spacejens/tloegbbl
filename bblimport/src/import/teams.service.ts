...
export class TeamsService {
  ...
  getTeams(): BblTeam[] {
    ...
    const teamViewFilenames = this.fileReaderService.listFiles('default.asp?p=tm&t=');
    for (const teamViewFilename of teamViewFilenames) {
      const teamId = teamViewFilename.slice(teamViewFilename.lastIndexOf('=') + 1);
      ...
      if (teamNameElements.length != 1) {
        throw new Error("Did not expect to find more than one team name for " + teamId);
      }
      ...
      } else if (coachElements.length === 2) { // TODO Merge one/many coaches cases into a single case to avoid code duplication
        teams.push({
          id: teamId,
          name: teamName,
          headCoach: {
